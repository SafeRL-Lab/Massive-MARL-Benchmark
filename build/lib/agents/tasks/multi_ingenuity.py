...
def compute_ingenuity_reward(obs_buf_1,obs_buf_2,obs_buf_3,obs_buf_4,pos_before_1,pos_before_2,pos_before_3,pos_before_4,
    ...
    die = torch.where((target_dist_1 > 8.0) | (target_dist_2 > 8.0) | (target_dist_3 > 8.0) | (target_dist_4 > 8.0), ones, die)
    die = torch.where((root_positions_1[:, 2] < 0.5) | (root_positions_2[:, 2] < 0.5) | (root_positions_3[:, 2] < 0.5) | (root_positions_4[:, 2] < 0.5), ones, die)
